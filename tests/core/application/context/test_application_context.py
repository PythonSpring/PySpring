import pytest
from py_spring.core.application.context.application_context import ApplicationContext, ApplicationContextConfig
...
class TestApplicationContext:
    ...
    def test_register_entities_correctly(self, app_context: ApplicationContext):
        ...
        assert "TestComponent" in app_context.component_cls_container and app_context.component_cls_container["TestComponent"] == TestComponent
        assert "TestController" in app_context.controller_cls_container and app_context.controller_cls_container["TestController"] == TestController
        assert "TestBeanCollection" in app_context.bean_collection_cls_container and app_context.bean_collection_cls_container["TestBeanCollection"] == TestBeanCollection
        assert "test_properties" in app_context.properties_cls_container and app_context.properties_cls_container["test_properties"] == TestProperties
    ...
    def test_register_invalid_entities_raises_error(self, app_context: ApplicationContext):
        """
    ...
    def test_retrieve_singleton_app_entities(self, app_context: ApplicationContext):
        ...
        app_context.singleton_component_instance_container["TestComponent"] = component_instance
        ...
        app_context.singleton_bean_instance_container["TestBeanCollection"] = bean_instance
        ...
        app_context.singleton_properties_instance_container["test_properties"] = properties_instance
        retrieved_properties = app_context.get_properties(TestProperties)
        assert retrieved_properties is properties_instance
        
    def test_inject_dependencies_for_components_and_controllers(self, app_context: ApplicationContext):
        """
        ...
        assert hasattr(TestComponent, 'test_nested_component') and isinstance(TestComponent.test_nested_component, TestNestedComponent)
        assert hasattr(TestController, 'test_component') and isinstance(TestController.test_component, TestComponent)
