...
class ApplicationContext:
    ...
    def get_sinleton_bean_instances(self) -> list[object]:
    ...
    def _inject_compoennt_dependencies(self, entity: Type[Component | RestController]) -> None:
        for attr_name, annotated_entity_cls in entity.__annotations__.items():
            if annotated_entity_cls in self.primitive_types:
                logger.warning(
                    f"[DEPENDENCY INJECTION SKIPPED] Skip inject dependecy for attribute: {attr_name} with dependency: {annotated_entity_cls.__name__} because it is primitive type"
                )
                continue
            ...
            if optional_component is not None:
                ...
                logger.success(
                f"[DEPENDENCY INJECTION SUCCESS FROM COMPONENT CONTAINER] Inject dependecy for attribute: {attr_name} with dependency: {annotated_entity_cls.__name__} singleton instance"
                )
                continue
            ...
            if optional_bean is not None:
                ...
                logger.success(
                f"[DEPENDENCY INJECTION SUCCESS FROM BEAN COLLECTION CONTAINER] Inject dependecy for attribute: {attr_name} with dependency: {annotated_entity_cls.__name__} singleton instance"
                )
                continue
            
            error_message = f"[DEPENDENCY INJECTION FAILED] Fail to inject dependecy for attribute: {attr_name} with dependency: {annotated_entity_cls.__name__}, consider register such depency with Compoent decorator"
    ...
    def inject_dependencies_for_component_container(self) -> None:
        for _component_cls_name, component_cls in self.component_cls_container.items():
            self._inject_compoennt_dependencies(component_cls)
    ...
    def inject_dependencies_for_controller_container(self) -> None:
        for _controller_cls_name, controller_cls in self.controller_cls_container.items():
            self._inject_compoennt_dependencies(controller_cls)
