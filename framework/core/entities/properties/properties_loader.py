...
class _PropertiesLoader:
    ...
    def __init__(self, properties_path: str, properties_classes: list[Type[Properties]]) -> None:
    ...
    def _load_classes_as_map(self) -> dict[str,Type[Properties]]:
        return {
            _cls.get_key(): _cls 
            for _cls in self.properties_classes
        } 
    ...
    def load_properties(self) -> dict[str,Properties]:
        ...
        for key, value in properties_dict.items():
            if key not in self.properties_class_map.keys():
                raise ValueError(f"[INVALID PROPERTIES KEY] Invalid properties key: {key}, please enter one of the following [{','.join(self.properties_class_map.keys())}]")
